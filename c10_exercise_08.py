"""
8. Modify the PDB file reader to check that the serial numbers on atoms start
at 1 and increase by 1. What should the modified function do if it finds a file
that doesn’t obey this rule?
"""
...
def read_molecule(reader: TextIO) -> list:
    ...
    while reading:
        ...
        if line.startswith('END'):
            ...
            key, sn, atom_type, x, y, z = line.split()
            if molecule == None:
                molecule = []
            molecule.append([atom_type, x, y, z])
            if serial_num != int(sn):
                print('Expected serial number {0}, but got {1}'. \
                      format(serial_num, sn))
            serial_num += 1
